...
## 2026-06-25 -- Market-Open Trade (XLB ADD post-MU beat)

| Date | Ticker | Side | Shares | Entry (avg) | Stop | Thesis | Target | R:R |
|------|--------|------|--------|-------------|------|--------|--------|-----|
| 2026-06-25 | XLB | BUY | 128 (add) | ~$51.63 | $46.485 (10% trail, hwm $51.65) | MU AMC blow-out = AI/semi risk-on bid; redeploy GNRC stop cash into existing Materials winner ("Leading" quadrant + Mfg PMI 55.7 49-mo high + sub-$80 WTI petrochem tailwind); size respects 25% per-position cap | $59.37 (+15%) | 2.5:1 |

**Notes:** MU AMC Wed beat-and-raise confirmed by tape: Thu 09:36 ET prints MTUM +3.97% to $341.63, XLK +2.38% to $187.43, XLI +1.64% to $183.79 (fresh hwm $183.25 from yesterday), XLV +0.76% to $154.51 (fresh hwm $154.70), XLB +0.60% to $51.65. Day P&L premkt-to-open +$2,042 (+2.05%); phase smashes through Jun 22 +$1,920 record to fresh +$1,433 (already +1.43%, on pace for new campaign hwm). Pre-market conditional rebuy fired (MU beat + raise = redeploy $8.5K). **Chose XLB add over LYB single-name** because (i) existing "Leading" quadrant winner with clean technical setup, (ii) clean liquidity (XLB spread $51.49/$51.50 vs LYB wide $51.77/$57.21 at open), (iii) Materials gets cyclical bid from Mfg PMI 55.7 + sub-$80 WTI petrochem tailwind, (iv) avoids strike-3 single-name risk after DELL fail + GNRC exit.

XLB position 363 -> 491 sh; fill ~$51.63 (cash $8,595 -> $1,986.50 = -$6,608.64); new blended avg cost $51.298554 (was $51.18168). 128-sh lot trail GTC armed @ $46.485 (10% trail, hwm $51.65 from entry); existing 198+165 sh lots unchanged on $47.979 trail (hwm $53.31). Deployment 91.33% -> 98.04% ($99,446 / $101,433) -- lean cash 1.96%, well above 85% floor. Sector mix: Materials (XLB) 18.45% -> 24.96% (under 25% per-position cap by design); Multi-sector (MTUM) 25.27% (drift over 25% per-position cap on mark-to-market, no add); Industrials (XLI) 23.50%; Tech (XLK) 12.01%; Healthcare (XLV) 12.20%; Energy 0% (closed). All sectors under 30% cap. Weekly count: 1/6.

**Rule checks at execution:** Positions after = 5 (≤6 ceiling, XLB is existing add not new ticker); weekly count 1/6 (≤6 cap); per-position post-add XLB 24.96% (≤25% cap, by design 4-sh below); per-sector Materials 24.96% (≤30% cap); incremental buy cost $6,608.64 = 6.52% of equity (well under 20% workflow check); PDT daytrade_count = 0/3; catalyst documented (2026-06-24 RESEARCH-LOG conditional rebuy queue); 10% trail GTC armed pre-step-7.

**Watch:** Fri Jun 26 = PCE inflation AM + Iran MOU formal signing Switzerland = dual late-week catalyst. MTUM $341.63 below hwm $345.59 (no fresh ratchet yet); GNRC slot closed (strike-1 Industrials trim accepted). XLI fresh hwm $183.25 -> trail ratcheted $164.628 -> $164.925. XLV fresh hwm $154.70 -> trail $138.3435 -> $139.23. Industrials at 23.50% room for adds, but no new single-name swing this week (GNRC just exited). EOD snapshot Day 41 (Thu) on next workflow.
